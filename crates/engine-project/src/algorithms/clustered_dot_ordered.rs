//! Clustered-dot ordered dither (`clustered_dot_ordered`).
...
/// Same schema as Bayer ordered dither (levels / scale / pixel size / angle).
const CLUSTERED_DOT_SCHEMA: &[ParamField] = &[
    ParamField::Slider {
        key: "levels",
        label: "Levels",
        min: 2.0,
...
/// Classical clustered-dot (newspaper-style) ordered dither.
pub struct ClusteredDotOrdered;

impl FilterAlgorithm for ClusteredDotOrdered {
    fn id(&self) -> AlgorithmId {
        AlgorithmId::new("clustered_dot_ordered")
    }
    ...
    fn apply(
        ...
        params.mode = DitherModeV2::ClusteredDotOrdered;
