//! Parity: Registry `random_dot_stipple` vs the stipple apply path.
...
use engine_project::filters::random_dot_stipple;
...
fn random_dot_params() -> DitherParamsV2 {
    DitherParamsV2 {
        mode: DitherModeV2::RandomDotStipple,
        pixel_size: 1,
        threshold_scale: 1.0,
...
#[test]
fn registry_parity_random_dot_stipple() {
    let params = random_dot_params();
    ...
    .expect("random_dot_stipple path");

    let mut registry = AlgorithmRegistry::new();
    register_all(&mut registry);
    let algo = registry
        .get(AlgorithmId::new("random_dot_stipple"))
        .expect("registered");
    assert!(matches!(
    ...
    filter.algorithm_id = Some("random_dot_stipple".into());
...
#[test]
fn random_dot_geometry_helpers() {
    assert!(!random_dot_stipple::random_dot_ink(0, 0, 0.0, 1.0, 1));
}
