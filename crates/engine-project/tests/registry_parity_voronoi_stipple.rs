//! Parity: Registry `voronoi_stipple` vs the stipple apply path.
...
fn voronoi_params() -> DitherParamsV2 {
    DitherParamsV2 {
        mode: DitherModeV2::VoronoiStipple,
...
#[test]
fn registry_parity_voronoi_stipple() {
    let params = voronoi_params();
    ...
    .expect("voronoi_stipple path");

    let mut registry = AlgorithmRegistry::new();
    register_all(&mut registry);
    let algo = registry
        .get(AlgorithmId::new("voronoi_stipple"))
        .expect("registered");
    assert!(matches!(
    ...
    filter.algorithm_id = Some("voronoi_stipple".into());
