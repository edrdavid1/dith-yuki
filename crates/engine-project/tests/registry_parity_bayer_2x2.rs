//! Parity: Registry `bayer_2x2` vs the legacy ordered-dither path.
...
fn bayer2x2_params() -> DitherParamsV2 {
    DitherParamsV2 {
...
#[test]
fn registry_parity_bayer_2x2() {
    let params = bayer2x2_params();
    ...
    let algo = registry
        .get(AlgorithmId::new("bayer_2x2"))
        .expect("bayer_2x2 registered");
    ...
    assert_eq!(
        ...
        "Bayer2x2 registry apply must match apply_ordered_with_cache_into"
    );
    ...
    assert_eq!(
        ...
        "Bayer2x2 dispatcher registry path must match apply_ordered_with_cache_into"
    );
}
