//! Parity: Registry `bayer_8x8` vs the legacy ordered-dither path.
...
fn bayer8x8_params() -> DitherParamsV2 {
    DitherParamsV2 {
...
#[test]
fn registry_parity_bayer_8x8() {
    let params = bayer8x8_params();
    ...
    let algo = registry
        .get(AlgorithmId::new("bayer_8x8"))
        .expect("bayer_8x8 registered");
    ...
    assert_eq!(
        ...
        "Bayer8x8 registry apply must match apply_ordered_with_cache_into"
    );
    ...
    assert_eq!(
        ...
        "Bayer8x8 dispatcher registry path must match apply_ordered_with_cache_into"
    );
}
