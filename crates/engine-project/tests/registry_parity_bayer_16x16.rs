//! Parity: Registry `bayer_16x16` vs the ordered-dither path.
...
fn bayer16x16_params() -> DitherParamsV2 {
    DitherParamsV2 {
...
#[test]
fn registry_parity_bayer_16x16() {
    let params = bayer16x16_params();
    ...
    let algo = registry
        .get(AlgorithmId::new("bayer_16x16"))
        .expect("bayer_16x16 registered");
    ...
    assert_eq!(
        ...
        "Bayer16x16 registry apply must match apply_ordered_with_cache_into"
    );
    ...
    filter.algorithm_id = Some("bayer_16x16".into());
    ...
    assert_eq!(
        ...
        "Bayer16x16 dispatcher registry path must match apply_ordered_with_cache_into"
    );
}
