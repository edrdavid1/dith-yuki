//! Parity: Registry `shiau_fan` vs the error-diffusion path.
...
#[test]
fn registry_parity_shiau_fan() {
    let env = Env::new();
    let params = dither_params(DitherModeV2::ShiauFan);
    env.assert_gpu(
        "shiau_fan",
        &FilterParams::DitherV2(params.clone()),
        GpuEligibility::Cpu(CpuCheckpointKind::ErrorDiffusion),
    );
    env.assert_ed("shiau_fan", params);
}
