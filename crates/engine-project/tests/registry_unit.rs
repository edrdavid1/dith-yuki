...
fn register_all_ids_unique() {
    ...
    assert!(
        registry.get_by_str("shiau_fan").is_some(),
        "Batch B must register shiau_fan"
    );
    assert!(
        registry.get_by_str("fan93").is_some(),
        "Batch B must register fan93"
    );
    assert!(
        registry.get_by_str("ostromoukhov").is_some(),
        "Batch C must register ostromoukhov"
    );
    assert!(
        registry.get_by_str("zhou_fang").is_some(),
        "Batch C must register zhou_fang"
    );
    assert!(
        registry.get_by_str("riemersma").is_some(),
    ...
    assert!(
        registry.get_by_str("voronoi_stipple").is_some(),
        "Batch E must register voronoi_stipple"
    );
    assert!(
        registry.get_by_str("random_dot_stipple").is_some(),
        "Batch E must register random_dot_stipple"
    );
    assert!(
