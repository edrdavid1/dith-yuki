//! Parity: Registry `line_screen` vs the line-screen apply path.
...
fn line_params() -> DitherParamsV2 {
    DitherParamsV2 {
        mode: DitherModeV2::LineScreen,
...
#[test]
fn registry_parity_line_screen() {
    ...
    .expect("line_screen path");

    let mut registry = AlgorithmRegistry::new();
    register_all(&mut registry);
    let algo = registry
        .get(AlgorithmId::new("line_screen"))
        .expect("registered");
    assert!(matches!(
    ...
    filter.algorithm_id = Some("line_screen".into());
...
#[test]
fn line_screen_geometry_helpers() {
    assert!(line_screen::line_screen_ink(4, 0, 1.0, 8.0, 0.0, 1.0));
    assert!(!line_screen::line_screen_ink(4, 0, 0.0, 8.0, 0.0, 1.0));
}
