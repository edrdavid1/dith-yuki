// CMYK angled-screen halftone — resident storage textures (Path B).
// Ports CPU apply_cmyk_halftone (ps=1, no palette) + v1 halftone.wgsl.
...
fn main(@builtin(global_invocation_id) gid: vec3<u32>) {
    ...
    var out = vec4<f32>(rgb.r, rgb.g, rgb.b, a);
    if (dither_a) {
        if (a <= 0.0) {
            out.a = 0.0;
        } else if (a >= 1.0) {
