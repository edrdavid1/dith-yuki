// Palette Guided ordered dither — resident storage textures (Path B).
// Ports CPU quantize_channel_guided + Bayer threshold (ps=1).
...
struct GuidedUniforms {
    tile: TileUniforms,
    // x=channel_levels, y=threshold_scale (z/w unused — primary uniform ≤32 B)
    params: vec4<f32>,
}
...
struct ChannelRanges {
    // xyzw = r_min, r_max, g_min, g_max
    rg: vec4<f32>,
    // xy = b_min, b_max
    b_pad: vec4<f32>,
}

@group(0) @binding(0) var<uniform> u: GuidedUniforms;
...
@group(0) @binding(4) var<storage, read> ranges: ChannelRanges;
...
fn quantize_channel_guided(value: f32, lo: f32, hi: f32, levels: f32, threshold: f32) -> f32 {
    let lv = max(levels, 2.0);
    let span = max(hi - lo, 1e-6);
    let normalized = clamp((value - lo) / span, 0.0, 1.0);
    let scaled = normalized * (lv - 1.0);
    ...
    step = clamp(step, 0.0, lv - 1.0);
    return lo + (step / (lv - 1.0)) * span;
}

fn guided_at(gid: vec3<u32>, threshold: f32) {
    ...
    var out = rgba;
    if (!is_gray) {
        out.r = quantize_channel_guided(rgba.r, ranges.rg.x, ranges.rg.y, levels, t);
        out.g = quantize_channel_guided(rgba.g, ranges.rg.z, ranges.rg.w, levels, t);
        out.b = quantize_channel_guided(rgba.b, ranges.b_pad.x, ranges.b_pad.y, levels, t);
    } else {
        let lum = luminance(rgba.r, rgba.g, rgba.b);
        out.r = quantize_channel_guided(lum, ranges.rg.x, ranges.rg.y, levels, t);
        out.g = quantize_channel_guided(lum, ranges.rg.z, ranges.rg.w, levels, t);
        out.b = quantize_channel_guided(lum, ranges.b_pad.x, ranges.b_pad.y, levels, t);
    }
    if (dither_a) {
        if (rgba.a <= 0.0) {
            out.a = 0.0;
...
fn bayer2_main(@builtin(global_invocation_id) gid: vec3<u32>) {
    ...
    guided_at(gid, sample_bayer(gx, gy, 2u));
}
...
fn bayer4_main(@builtin(global_invocation_id) gid: vec3<u32>) {
    let gx = i32(u.tile.tile_offset.x + gid.x);
    let gy = i32(u.tile.tile_offset.y + gid.y);
    guided_at(gid, sample_bayer(gx, gy, 4u));
}
...
fn bayer8_main(@builtin(global_invocation_id) gid: vec3<u32>) {
    let gx = i32(u.tile.tile_offset.x + gid.x);
    let gy = i32(u.tile.tile_offset.y + gid.y);
    guided_at(gid, sample_bayer(gx, gy, 8u));
}
