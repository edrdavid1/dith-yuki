...
> A8 occupancy (preview+A2): ~19% on 3072²/1080p.
...
- **Primary Control:** `DITHER_GPU_PREVIEW=1` = **cold GPU compute** opt-in (debug/soak). Warm resident Composite may download without that flag (auto-dispatch A1). Product UI has no GPU toggle (A4).
