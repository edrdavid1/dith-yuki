...
- [ ] **Phase 6: Layers & Filters Domain Extraction**
  ...
  - [ ] Migrate tile caches (`tile_cache`, `scheduler`, `ed_frontier`, etc.) from `AppState` into `TileState`
  - [ ] Move layer commands (`get_layer_tree`, `add_layer`, `remove_layer`, `reorder_layer`, `set_layer_props`) to `commands/layers.rs` and delegate to `LayerService`
  - [ ] Move filter commands (`add_filter`, `update_filter`, `remove_filter`, `reorder_filter`) to `commands/filters.rs` and delegate to `FilterService`
  - [ ] Re-export all layer DTOs (`LayerNodeDto`, `AddLayerRequest`, `SetLayerPropsRequest`, `ReorderLayerRequest`, `LayerPropsPatchDto`) in `commands/mod.rs`
  - [ ] **BEFORE COMMITTING**: Diff against `pre-refactor-commands` baseline to verify invalidation cascade order for all layer/filter operations
  - [ ] Remove extracted commands from `commands.rs`
  - [ ] Verify: `cargo build --all && cargo test --all`

- [ ] **Phase 7: Palette & Color Lab Domain Extraction**
  - [ ] Create `src-tauri/src/services/palette_service.rs`
  - [ ] Move synchronous palette CRUD commands to `commands/palette.rs` and delegate to `PaletteService`
  - [ ] **Handle `generate_palette` separately as an async command**: retain `async fn generate_palette(...)` signature, preserve layer tile sampling (MedianCut/KMeans) and async execution flow without forcing into synchronous service pattern
  - [ ] Move pure color conversion commands (`generate_ramp_palette`, `generate_harmony_palette`, `colors_to_oklab`, `get_palette_oklab`) into `commands/color_lab.rs`
  - [ ] **BEFORE COMMITTING**: Diff extracted palette/color commands against `pre-refactor-commands` baseline
  - [ ] Remove extracted commands from `commands.rs`
  - [ ] Verify: `cargo build --all && cargo test --all`

- [ ] **Phase 8: Document Domain Extraction & File Deletion**
  - [ ] Create `src-tauri/src/services/document_service.rs`
  - [ ] Move document lifecycle commands (`load_image`, `create_document`, `new_document`, `list_open_documents`, `set_active_document`, `close_document`, `get_document_snapshot`, `open_project`, `save_project`, `save_project_as`, `export_pattern`, `import_pattern`, `export_image`, `import_image_layer`, `is_document_dirty`, `get_recent_files`) to `commands/document.rs` and delegate to `DocumentService`
  - [ ] Re-export document DTOs (`DocumentResponse`, `DocumentChangedPayload`, etc.) in `commands/mod.rs`
  - [ ] **BEFORE COMMITTING**: Diff against `pre-refactor-commands` baseline to ensure document dirty flag semantics and recent files write-on-success behavior are preserved
  - [ ] Confirm `commands.rs` is completely empty and delete `src-tauri/src/commands.rs`
  - [ ] Update `src-tauri/src/main.rs` to register commands from `commands::document::*`, `commands::layers::*`, etc.
  - [ ] Final verification: `cargo build --all && cargo test --all`

---

## Final Definition of Done Checklist

- [ ] `src-tauri/src/commands.rs` is removed from repository.
- [ ] Every public `#[tauri::command]` signature (function name, parameter names/types, return type) is 100% identical to the `pre-refactor-commands` baseline.
- [ ] All DTO structs (`SelectionChangedPayload`, `LayerNodeDto`, `DocumentResponse`, etc.) are re-exported in `commands/mod.rs` so module paths and macro imports remain unbroken.
- [ ] All IPC handlers in `commands/*.rs` are short wrappers (3–8 lines).
- [ ] `AppState` cleanly embeds `TileState`, `UiState`, and `HistoryState`.
- [ ] `generate_palette` remains properly async and non-blocking for layer tile sampling.
- [ ] No changes were made to `frontend/src/shared/ipc/`.
- [ ] All behavior preservation invariants (undo order, invalidation order, dirty flag, recent files logging) verified via `git diff pre-refactor-commands` BEFORE committing each phase.
- [ ] Automated tests pass: `cargo test --all`.
